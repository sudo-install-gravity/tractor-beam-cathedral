...
**Updated 2026-08-10 (`indexer` T-12.5 pass — lands after T-12.6, both dated the same
day):** documentation-only change, `docs/PHYSICS.md` and `docs/CLAIMS.md` — **no `.py`
file touched, so no Equation Registry row's function/test/source columns move.**
PHYSICS.md's last three `[UNVERIFIED]` markers are gone (linear memory §4, the array-factor
family §5, geodesic deviation §6), and every Category B claim now carries a derivation
with an explicit reducing limit: §3.1 (B-2), §5.1 (B-1), §6.1 (B-5, the one claim that was
genuinely still open), §8.1 (B-3), §9.1 (B-4), and a new §10 ledger covering all nine
claims, including §10.1 (B-7), §10.2 (B-8) and §10.3 (B-9). **The three re-sourced PHYSICS.md
citations were checked against this registry and already agree — no drift found:** linear
memory → Favata eq. 10k matches EQ-026 (line 91); array factor/grating/beamwidth →
Orfanidis ch. 19 eqs. 19.4.1/19.9.6/19.7.6 matches EQ-016/EQ-015/EQ-017 (lines 78–82);
geodesic deviation → [FH] eq. 3.11 matches EQ-045 (line 110). `CLAIMS.md` A-6 and A-8 were
correspondingly re-sourced away from chapter references (MTW §37.2, Balanis ch. 6); this
registry never carried either chapter reference itself (checked directly, not inferred), so
nothing here needed correcting to match. Breadcrumbs to the two new PHYSICS.md sections were
added below to the three EQ rows that already name the claim they discharge (EQ-047, EQ-048
→ §5.1; EQ-053 → §8.1). Two findings recorded from this pass: a registry structural gap
(B-8/B-9 have no Category A row to reduce to — see the note at the end of the Sprint 14
citation block below) and a float64-resolution finding added to the existing focusing/
steering-degeneracy row in §3.
...
| EQ-047 | **Spin-2 tensor array superposition** `h_ij(n̂) = Σ_n Λ_ij,kl(n̂) Q^(n)_kl w_n exp(i k·r_n)` | [B] eq. 2 (VERIFIED at EQ-005) per element, superposed per [ADR-0003](adr/0003-spin2-superposition.md) | `array/beamform.py:superpose_tt` (T-6.5) | `tests/unit/test_superposition.py` | DERIVED — ⚠️ **the project's highest-risk equation** (CLAUDE.md rule 4). Sums TT tensors along **one common `n̂`** and raises inside the Fraunhofer distance; discharges claim B-1 (derivation: `PHYSICS.md` §5.1) |
| EQ-048 | Spin-2 polarization-mismatch factor `cos(2Δψ)` | Derived in [ADR-0003](adr/0003-spin2-superposition.md) (claim B-1), from [B] eq. 2 | `array/beamform.py:mismatch_loss` (T-6.6) | `tests/unit/test_superposition.py` | DERIVED — ⚠️ **maximal at 45°, not 90°; elements 90° apart CANCEL where spin-1 intuition predicts 2× power** — derivation: `PHYSICS.md` §5.1 |
...
⚠️ **Finding, T-12.5 (2026-08-10) — the claims registry's promotion rule has a gap for
elementary-mechanics claims, and it is not an INDEX.md defect.** `CLAIMS.md` B-8
(`required_miss_distance`'s gravitational-focusing criterion, the "Gravitational focusing"
bullet above) previously named its "Reduces to" claim as **A-6** — the geodesic-deviation
claim, about GW tidal strain, which has no bearing on Earth's gravitational focusing of a
hyperbolic impactor. This registry itself never repeated that mislabeling — the
"Gravitational focusing" bullet above cites no Category A claim at all — so nothing here
needed correcting. `CLAIMS.md` now reads B-8 → "— (elementary two-body mechanics; no
Category A claim applies)", and the gap is real, not merely a wrong ID: this registry's own
§1 has no numbered claim for elementary two-body mechanics for B-8 (and B-9, which inherits
the gap through its dependence on B-8's `b_req`) to reduce to. Recorded per rule 8 rather
than closed by inventing a claim ID to point at. See `PHYSICS.md` §10.2/§10.3.
...
| OQ-8 | Is the R5 scoping table's "wall vanishes" configuration (10⁹ kg, 1 km, 1 MHz) physically admissible, given that its rod-model tip speed works out to `v/c ≈ 10.5`? | **Open, unreviewed** — [ADR-0009](adr/0009-magnitude-wall-relativistic-validity.md), 2026-08-17 external review, not yet triaged by `researcher`/`code-reviewer`. If confirmed, affects how C-2's `−6.75` decade figure and `R5.json`'s attribution of that row's infeasibility to C-1 (the transducer) should be stated — the finding argues that attribution is wrong above `f ≈ 95.4 kHz`, where the source kinematics are superluminal rather than merely unbuilt. Also raises a candidate fourth wall (material tensile-strength ceiling on `(M, R, ω)`), currently absent from the ledger |
