...
| EQ-035 | Spin-2 polarization basis `(e₊, e_×)` transverse to `n̂` | [FH] eq. 2.22 | `propagate/polarization.py:polarization_basis` (T-5.1) | `tests/unit/test_polarization.py` | ⚠️ **PARTIALLY VERIFIED 2026-08-03 — the cited equation says something narrower than this row claims.** [FH] eq. 2.22 was read directly and is `h^TT_xx = −h^TT_yy ≡ h₊(t−z)`, with eq. 2.23 giving `h^TT_xy = h^TT_yx ≡ h_×(t−z)`. Those define the **scalars** `h₊`/`h_×` from TT metric components; they do **not** define the **basis tensors** `e₊`, `e_×` that `polarization_basis` returns. The construction is standard and almost certainly right, but this citation does not establish it. Needs a source that states the basis tensors, or a Category B re-label as our construction. **Found while correcting EQ-040 — same paper, adjacent defect** |
...
| EQ-038 | TT tensor → polarization scalars, `(h₊, h_×) = ½ e_A : h_ij` | [FH] eq. 2.22, inverted | `propagate/polarization.py:decompose` (T-5.2) | `tests/unit/test_polarization.py` | DERIVED — ✅ **underlying citation now checked (2026-08-03).** [FH] eq. 2.22/2.23 do define exactly this scalar⇄tensor correspondence (`h^TT_xx = −h^TT_yy ≡ h₊`, `h^TT_xy ≡ h_×`), so this row is sound — but it should cite **2.22 *and* 2.23**, since `h_×` comes from 2.23. See EQ-035 for the part of that paper's §2.2 that does *not* say what was claimed |
| EQ-039 | Polarization scalars → TT tensor, `h_ij = h₊e₊ + h_×e_×` | [FH] eq. 2.22 | `propagate/polarization.py:recompose` (T-5.2) | `tests/unit/test_polarization.py` | DERIVED — ✅ **underlying citation now checked (2026-08-03).** [FH] eq. 2.22/2.23 do define exactly this scalar⇄tensor correspondence (`h^TT_xx = −h^TT_yy ≡ h₊`, `h^TT_xy ≡ h_×`), so this row is sound — but it should cite **2.22 *and* 2.23**, since `h_×` comes from 2.23. See EQ-035 for the part of that paper's §2.2 that does *not* say what was claimed |
...
**Still unverified, and now the priority: [FH] eq. 3.11 (EQ-045), [FH] eq. 4.30/4.35
(EQ-041/042), and [B] eq. 123a (EQ-044).** EQ-040 is the reason to check these rather than
assume them: a plausible equation number, in the right paper, pointed at the wrong equation,
and nothing but reading the source caught it.
