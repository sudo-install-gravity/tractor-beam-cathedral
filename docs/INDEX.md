...
| `array/focus.py` | Spatiotemporal focusing | **partially live** — `focal_phases` (T-9.5). `focused_field` (T-9.6), `focus_trajectory` (T-9.7), `spot_size` (T-10.1), `dwell_time` (T-10.3) **not yet implemented** | `geometry`, `constants`, `core/validation` |
...
evaluation `focused_field` (T-9.6, `opus`, **critical path**) does not. Note that
`superpose_tt` assumes the **far field** — it takes a single common observation direction and
raises inside the Fraunhofer distance — so T-9.6 cannot simply call it for a near-field focus.
That tension is unresolved and is the first thing the next `opus` session must settle; see
`HANDOVER.md`.
...
| Diffraction limit `w ≈ λr/D` (T-10.2) | `array/focus.py:spot_size` | Not implemented — blocked on T-10.1, itself blocked on T-9.6. **Citation is already verified and waiting**: the −3 dB (FWHM) coefficient for a uniformly-illuminated circular aperture is `1.029 λ/D`, from the root `x = 1.61633` of `2J₁(x)/x = 1/√2` — self-auditable via `scipy.special.j1`, needing no textbook page. Do **not** use the Rayleigh 1.22 factor: that is the first Airy null, not the −3 dB width |
...
**Three tasks are blocked, all machine-readable in their `deps` field** so the scheduler
excludes them *and names the reason*: T-2.9 (repo made public), T-12.2 (exact Peters 1964
equation number), T-4.5 (citable uniform-sphere `l=2` form factor — new, see OQ-7). Nine
further tasks are transitively stranded behind them and are listed by `--plan`.
...
| OQ-3 | At what R/λ does the long-wavelength quadrupole approximation fail badly enough to matter? | Determines whether `bodies/` finite-size corrections are a refinement or a requirement. **T-4.5 is now BLOCKED on this** — see OQ-7 |
| OQ-7 (new) | What citable numbered equation gives the finite-size form factor for the **mass quadrupole** (`l = 2`) of a uniform-density sphere? | Blocks T-4.5, and transitively T-4.7/4.8/4.9. `researcher` returned **UNVERIFIED** and found the task's premise was wrong: `sin(kR)/(kR)` (leading term `1 − (kR)²/6`) is `l = 0` **spin-1 antenna machinery** — the exact trap of CLAUDE.md rule 4 — and `3j₁(kR)/(kR)` (`1 − (kR)²/10`) is the *total-mass monopole*, not the quadrupole. Volume-integrating `j_l(kr)` against `r^{l+2}dr` gives `1 − (kR)²(l+3)/[2(2l+3)(l+5)]`, so `l = 2` yields `1 − 5(kR)²/98` — but that is a **derivation with no numbered equation found**. Thorne, *Rev. Mod. Phys.* 52:299 (1980) is the likely primary source, paywalled, equation number unconfirmed. Needs SPIKE-4.5 at `opus`, producing an ADR |
